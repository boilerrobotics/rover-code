...
for section, odrv in odrvs.items(): # Checking errors
    ...
    print(f'  current = {odrv.ibus:5.2f} A')
...
for section, odrv in odrvs.items(): # Test run
    ...
    odrv.axis0.controller.input_vel = test_run_speed
    odrv.axis1.controller.input_vel = test_run_speed
    for _ in range(test_run_duration):
        print(f'  voltage = {odrv.vbus_voltage:5.2f} V')
        print(f'  current = {odrv.ibus:5.2f} A')
        time.sleep(1)
